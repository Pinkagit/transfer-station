...
class WechatJSSDK {
  ...
  signSignature (newSignConfig) {
    ...
    wx.ready(() => {
      ...
      this.setOriginWx()
      // debug && window.alert('sign signature finished...');
      config.success && config.success.call(this, this)
    })
    wx.error((err) => {
      debug && window.alert('sign error: ' + JSON.stringify(err))
      this.setOriginWx()
  ...
  loadScript () {
    ...
    ele.onerror = (err) => {
      ...
      console.error(err)
      this.debug && window.alert('Cannot load wechat script!')
    }
  ...
  callWechatApi (apiName, config) {
    ...
    if (this.config.jsApiList.indexOf(apiName) < 0) {
      debug && window.alert('the wechat api [' + apiName + '] you call was not registered, \npls add the api into your [jsApiList] config')
      return this
    }
    const wx = this.getOriginalWx()
    let customAPI = wx[apiName]
    if (!customAPI || typeof customAPI !== 'function') {
      debug && window.alert('no such api [' + apiName + '] found!')
      return this
    }
    customAPI(config)
    return this
